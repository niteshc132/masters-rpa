...
(async () => {
  ...
  const links = JSON.parse(
    fs.readFileSync("advisor_names_wellington.json", "utf8")
  );
  ...
  for (let i = 0; i < links.length; i += concurrency) {
    linkChunks.push(links.slice(i, i + concurrency));
  }
  for (const chunk of linkChunks) {
    const promises = chunk.map(async (link) => {
      ...
      const teamMembers = await page?.evaluate(() => {
        const teamList = document.querySelectorAll(
          ".advisors-profile-team-name"
        );
        const members = [];
        if (teamList) {
      ...
      const socials = socialLinks?.reduce((acc, curr) => {
